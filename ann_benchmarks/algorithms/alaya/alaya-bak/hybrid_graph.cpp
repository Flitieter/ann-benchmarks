...
void HybridGraph::BatchSearch(float *base, float *base_by_time, float *base_by_full_time, float *vecs, float *metas, int nq, uint32_t *ids, const SearchParams &params,int nb)
{
...
//#pragma omp parallel for schedule(dynamic)
    for (size_t i = 0; i < nq; ++i) {
        ...
        if (type == 0 || type == 2) {
            ...
            cout<<"id: "<<i<<" "<<"candidate_dists: "<<candidate_dists.size()<<"\n";
            maxPQIFCS<float>point_dist(k);
