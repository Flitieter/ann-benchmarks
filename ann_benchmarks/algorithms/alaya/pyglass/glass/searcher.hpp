...
//      assert(ef == 32);
  ...
  // void SearchOpt(const float *query, int k, int refine, int *res_id, float *res_dist) {
  //   auto computer = quant.get_computer(q);
  //   searcher::LinearPool<typename Quantizer::template Computer<0>::dist_type> pool(nb, std::max(k, ef), k);
  //   graph.initialize_search(pool, computer);
  //   SearchOptImpl(pool, computer);
  //   quant.reorder_dist(pool, query, res_id, res_dist, refine);
  // }

  // template <typename Pool, typename Computer>
  // void SearchOptImpl(Pool &pool, const Computer &computer) {
  //   while (pool.has_next()) {
  //     auto u = pool.pop();
  //     graph.prefetch(u, graph_po);
  //     for (int i = 0; i < po; ++i) {
...
inline std::unique_ptr<SearcherBase> create_searcher(const Graph<int> &graph, const std::string &metric,
                                                     int level = 1) {
  ...
  if (level == 0) {
    ...
    if (m == Metric::L2) {
      std::cout << "Create SQ8S Searcher" << std::endl;
      return std::make_unique<Searcher<SQ8SymmetricQuantizer<Metric::L2>>>(graph);
    } else if (m == Metric::IP) {
      return std::make_unique<Searcher<SQ8SymmetricQuantizer<Metric::IP>>>(graph);
    } else {
      printf("Metric not supported\n");
      return nullptr;
    }
