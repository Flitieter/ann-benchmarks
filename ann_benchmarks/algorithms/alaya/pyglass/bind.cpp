...
void fit(float *vec_data, int data_num, int data_dim) {
  ...
  searcher = (glass::Searcher<glass::SQ8SymmetricQuantizer<glass::Metric::L2>>
                  *)(new glass::Searcher<
                     glass::SQ8SymmetricQuantizer<glass::Metric::L2>>(
      hnsw->GetGraph()));

  auto hnsw_s =
      (glass::Searcher<glass::SQ8SymmetricQuantizer<glass::Metric::L2>> *)
          searcher;
...
void batch_query(float *query, int query_num, int dim, int k, int ef,
                 int rerank_k, unsigned *res) {
  // printf("init search\n");
  std::vector<std::vector<int>> tmp_id(query_num, std::vector<int>(ef));
  std::vector<glass::searcher::LPool<float>> res_pool(
      query_num, glass::searcher::LPool<float>(rerank_k));
  data = Data.data();
  ...
  for (size_t q = 0; q < query_num; ++q) {
    ...
    for (size_t i = 0; i < rerank_k; ++i) {
      ...
      //         std::cout<<"\n";
      //         std::cout<<"dist: "<<glass::L2SqrRef(cur_query, data + ids[i] *
      //         dim, dim)<<"\n";
      res_pool[q].insert_back(
          ids[i], glass::L2SqrRef(cur_query, data + ids[i] * dim, dim));
    }
    //     std::cout<<"DONE!\n";
    res_pool[q].sorted();
    //     std::cout<<"DONE!\n";
    for (size_t i = rerank_k; i < ef; ++i) {
      res_pool[q].emplace_insert(
          ids[i], glass::L2SqrRef(cur_query, data + ids[i] * dim, dim));
    }
