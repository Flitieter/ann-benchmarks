...
inline void ReadSortedBaseTimestamp(vector<float> &Sources, uint32_t N, int Dim,vector<float> &vecs, vector<float> &labels, vector<float> &timestamps, vector<uint32_t> &sorted_base_ids,
...
//    ifs.read((char *)&N, sizeof(uint32_t));
    ...
    auto tmp_labels = std::make_unique<float[]>(N);
    auto tmp_timestamps = std::make_unique<float[]>(N);
    std::copy(Sources.begin(),Sources.end(),tmp_vecs.get());
    vecs.resize(N * vec_dim);
    labels.resize(N);
    timestamps.resize(N);
...
inline void ReadSortedQuery(vector<float> &Sources, uint32_t N, vector<float> &vecs, vector<float> &metas, vector<uint32_t> &sorted_ids,
